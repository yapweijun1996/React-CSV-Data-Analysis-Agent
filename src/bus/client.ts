...
const loadWorkerUrl = async (): Promise<string | null> => {
    ...
    if (!workerUrlPromise) {
        workerUrlPromise = import('../graph/runner?worker&url').then(
            (mod: WorkerUrlModule) => mod.default,
        );
    }
...
export class GraphBus {
    ...
    ensureWorker(): void {
        ...
        loadWorkerUrl()
            ...
            .catch(error => {
                console.error('Failed to initialize graph worker:', error);
    ...
    private handleWorkerError = (errorEvent: ErrorEvent) => {
        const payload: GraphWorkerEvent = {
            type: 'graph/error',
            message: errorEvent.message || 'Graph worker error',
            timestamp: Date.now(),
        };
...
export const graphBus = new GraphBus();
